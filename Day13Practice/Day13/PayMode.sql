...
BEGIN
	Declare
		...
		@paymode varchar(30) 
	BEGIN
		if exists(select * from sqlpractice.dbo.Policy WHERE PolicyId = @policyId)
		BEGIN
		select	@appNumber = AppNumber, @annualPremium = AnnualPremium, 
			@paymode = dbo.fnPaymode(paymentModelID)
			from sqlpractice.dbo.Policy WHERE PolicyId = @policyId
